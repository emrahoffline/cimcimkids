...
if [ -f "$LOCAL_ENV" ]; then
  ...
  for KEY in NEXTAUTH_SECRET GOOGLE_CLIENT_ID GOOGLE_CLIENT_SECRET SMTP_HOST SMTP_PORT SMTP_USER SMTP_PASS SMTP_FROM ORDER_NOTIFICATION_EMAIL ADMIN_EMAILS IYZICO_CARD_ENABLED IYZICO_API_KEY IYZICO_SECRET_KEY IYZICO_URI IYZICO_CALLBACK_ORIGIN BIEN_USERNAME BIEN_PASSWORD BIEN_TEST COMPANY_TITLE COMPANY_TCKN COMPANY_VKN COMPANY_TAX_OFFICE COMPANY_ADDRESS COMPANY_DISTRICT COMPANY_CITY COMPANY_COUNTRY COMPANY_EMAIL COMPANY_WEBSITE COMPANY_PHONE COMPANY_MERSIS EFATURA_AUTO_ISSUE EFATURA_ARCHIVE_SERIES EFATURA_INVOICE_SERIES EFATURA_PROFILE NAVLUNGO_USERNAME NAVLUNGO_PASSWORD NAVLUNGO_SENDER_ADDRESS_ID NAVLUNGO_TEST NEXT_PUBLIC_GOOGLE_MERCHANT_ID NEXT_PUBLIC_GA_MEASUREMENT_ID; do
